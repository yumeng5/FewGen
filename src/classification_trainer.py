...
class FewGenTrainer(transformers.Trainer):
    """
    ...
    def __init__(self,
        ...
        super(FewGenTrainer, self).__init__(model, args, data_collator, train_dataset, eval_dataset, 
    ...
    def temporal_ensemble(self):
        ...
        self.train_dataset.ensemble_pred = (1-self.args.momentum)*pred_probs + self.args.momentum*self.train_dataset.ensemble_pred
        self.train_dataset.ensemble_pred = self.train_dataset.ensemble_pred
        pred_probs = self.train_dataset.ensemble_pred / (1-self.args.momentum**self.train_dataset.ensemble_count)
        for i, feature in enumerate(self.train_dataset.all_features):
            feature.update_label(list(pred_probs[i]))
        return pred_probs
    ...
    def update_train_data(self):
        ...
        # No filter of training data; directly update ensemble predictions
        if self.args.threshold == 0:
            ...
            pred_probs = self.temporal_ensemble()
            max_probs = np.amax(pred_probs, axis=-1)
            predictions = np.argmax(pred_probs, axis=-1)
            labels = np.array([feature.label for feature in self.train_dataset.features])
            select_pos = (labels == predictions) & (max_probs > self.args.threshold)
            for i in range(self.train_dataset.num_labels):
                num_train = np.sum((labels == i) & select_pos)
                # If there are too few training data after filtering for some class, use half of the entire training data sorted by confidence
                if num_train < 16:
    ...
    def train(self, model_path=None, dev_objective=None):
        ...
        while True:
            ...
            if self.args.momentum > 0:
                ...
                # Update dataloader if filtering training samples
                if self.args.threshold > 0:
                    train_dataloader = self.get_train_dataloader()
            ...
            for step, inputs in enumerate(epoch_iterator):
                ...
                if self.args.evaluate_during_training and (self.global_step % self.args.eval_steps == 0):
                    output = self.evaluate(eval_dataset=self.eval_dataset, desc="Evalution")
                    metrics = output.metrics
                    objective = self.dev_objective(metrics)
                    print(f"Eval objective: {objective}")
